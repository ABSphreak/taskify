...
const TaskAddModal = ({ isOpen, setIsOpen, groupId }: { isOpen: boolean; setIsOpen: Dispatch<SetStateAction<boolean>>; groupId: string }) => {
	const { addTask } = useContext(TaskGroupContext);

	const handleAdd = (e: React.FormEvent<HTMLFormElement>) => {
		e.preventDefault();
		const title = e.currentTarget.elements.namedItem('title') as HTMLInputElement;
		addTask?.(groupId, title.value);
		setIsOpen(false);
	};
	...
	return (
		<Dialog open={isOpen} onClose={() => setIsOpen(false)} className="fixed inset-0 z-10 overflow-y-auto filter backdrop-blur-sm">
			...
			<div className="absolute p-4 -translate-x-1/2 -translate-y-1/2 bg-gray-700 rounded-md top-1/2 left-1/2">
				<Dialog.Title className="text-white">Add Task</Dialog.Title>
				<Dialog.Description className="sr-only">Add Modal</Dialog.Description>

				<form className="flex flex-col items-start gap-2 mt-5" onSubmit={e => handleAdd(e)}>
